...
class InitClusteringManager:
    ...
    COHESION_THRESHOLD = 0.78  # 凝集度の閾値（元の0.85と緩和後の0.70の中間値）
    MERGE_SIMILARITY_THRESHOLD = 0.83  # クラスタ間類似度の閾値（元の0.90と緩和後の0.75の中間値）
    ...
    def _get_folder_name(self, captions: list[str], extra_stop_words: list[str]) -> str:
        """
        TF-IDFを使用してフォルダ名を決定する（最上位1語を日本語に翻訳）
        """
        if not captions:
        ...
        # 最上位の単語を取得
        top_word = important_words[0][0]
        
        # 日本語に翻訳
        try:
            from deep_translator import GoogleTranslator
            translator = GoogleTranslator(source='en', target='ja')
            name = translator.translate(top_word)
        except Exception as e:
            print(f"⚠️ 翻訳エラー ({top_word}): {e}")
            # 翻訳失敗時は元の英単語を使用
            name = top_word
    ...
    def clustering_dummy(
        ...
        """
        ダミークラスタリング関数（folder_nameベース）
        folder_nameごとにリーフフォルダを作成し、トップディレクトリに並列配置
        folder_name=NULLの画像は「未分類」フォルダにまとめる
        """
        print(f"\n{'='*80}")
        print(f"clustering_dummy (folder_nameベース) 開始")
        print(f"{'='*80}\n")
        
        # === ステップ1: clustering_idとfolder_nameのマッピングを取得 ===
        print(f"📋 ステップ1: データベースからfolder_nameを取得")
        print(f"{'='*80}\n")
        
        # clustering_idのリストを作成
        clustering_id_list = list(clustering_id_dict.keys())
        print(f"  - 対象clustering_id数: {len(clustering_id_list)}")
        
        # データベースからfolder_nameを取得
        from db_utils.commons import create_connect_session
        from db_utils.images_queries import get_folder_names_by_clustering_ids
        ...
        folder_name_dict = get_folder_names_by_clustering_ids(connect_session, clustering_id_list)
        print(f"  - 取得したfolder_name数: {len(folder_name_dict)}")
        
        # === ステップ2: folder_nameごとにグループ化 ===
        print(f"\n📋 ステップ2: folder_nameごとにグループ化")
        print(f"{'='*80}\n")
        
        folder_groups = {}  # {folder_name: [clustering_ids]}
        uncategorized = []  # folder_name=NULLの画像
        
        for clustering_id, metadata in zip(sentence_name_db_data.get('ids', []), sentence_name_db_data.get('metadatas', [])):
            if clustering_id in sentence_id_dict:
                actual_clustering_id = sentence_id_dict[clustering_id]['clustering_id']
                path = metadata.path if hasattr(metadata, 'path') else metadata.get('path', 'N/A')
                
                # folder_nameを取得
                folder_name = folder_name_dict.get(actual_clustering_id)
                
                if folder_name is None or folder_name == '':
                    uncategorized.append((actual_clustering_id, path))
                else:
                    if folder_name not in folder_groups:
                        folder_groups[folder_name] = []
                    folder_groups[folder_name].append((actual_clustering_id, path))
        
        print(f"  - フォルダ数: {len(folder_groups)}")
        print(f"  - 未分類画像数: {len(uncategorized)}")
        
        for folder_name, items in list(folder_groups.items())[:5]:
            print(f"    📁 [{folder_name}]: {len(items)}個の画像")
        if len(folder_groups) > 5:
            print(f"    ... 他{len(folder_groups) - 5}フォルダ")
        
        # === ステップ3: 各folder_nameをリーフフォルダとして作成 ===
        print(f"\n📋 ステップ3: folder_nameごとにリーフフォルダを作成")
        print(f"{'='*80}\n")
        ...
        for folder_name, items in folder_groups.items():
            ...
            for clustering_id, path in items:
        ...
        # 未分類フォルダ（folder_name=NULLの画像）
        if uncategorized:
            folder_id = Utils.generate_uuid()
            folder_data = {}
            
            for clustering_id, path in uncategorized:
                folder_data[clustering_id] = path
            ...
            print(f"  📁 [未分類] (folder_name=NULL)")
            print(f"     - folder_id: {folder_id}")
            print(f"     - 画像数: {len(folder_data)}")
        
        print(f"\n📊 合計: {len(leaf_folders)}個のリーフフォルダを作成")
        
        # === ステップ4: トップフォルダでラップ（フラット構造）===
        print(f"\n📋 ステップ4: トップフォルダでラップ（フラット構造）")
        print(f"{'='*80}\n")
        
        top_folder_id = Utils.generate_uuid()
        display_name = overall_folder_name if overall_folder_name else "フォルダ名ベース分類"
        
        # リーフフォルダを直接トップフォルダの配下に配置（フラット構造）
        wrapped_result = {
            top_folder_id: {
                "data": leaf_folders,
                "parent_id": None,
        ...
        print(f"   - 直下のリーフフォルダ数: {len(leaf_folders)}")
        print(f"   - 構造: フラット（カテゴリフォルダなし）")
        ...
        print(f"\n{'='*80}")
        print(f"clustering_dummy (folder_nameベース) 完了")
        print(f"{'='*80}\n")
    ...
    def clustering(
        ...
        # ========================================
        # 第1段階: usage + category でクラスタリング（大カテゴリ分類）
        # ========================================
        print("\n【第1段階】usage + category でクラスタリング")
        ...
        # クラスタ数を5個に固定
        overall_cluster_num = 5
        
        print(f"  caption全体クラスタ数: {overall_cluster_num} (固定)")
        ...
        # 第1階層は必ず5個で固定（マージ処理なし）
        print(f"  第1階層クラスタ数: {len(overall_clusters)} (固定、マージなし)")
        ...
        for overall_idx, sentence_ids_in_overall in overall_clusters.items():
            ...
            # usage+categoryフォルダ名を兄弟フォルダとの差別化を考慮して決定
            ...
            print(f"\n【第2段階】nameでクラスタリング (第1段階クラスタ {overall_idx}: {overall_folder_name_tfidf})")
            
            # ========================================
            # 第2段階: name でクラスタリング
            # ========================================
            name_data = self._sentence_name_db.get_data_by_sentence_ids(sentence_ids_in_overall)
            
            # nameの埋め込みを取得
            name_embeddings = name_data['embeddings']
            ...
            if name_cluster_num <= 1:
                name_clusters = {0: sentence_ids_in_overall}
            else:
                embeddings_array = np.array([emb.tolist() if hasattr(emb, 'tolist') else emb for emb in name_embeddings])
            ...
            name_clusters = self._merge_similar_clusters(name_clusters, name_embeddings, image_embeddings_dict)
            ...
            for name_idx, sentence_ids_in_name in name_clusters.items():
                ...
                for sentence_id in sentence_ids_in_name:
                    for i, sid in enumerate(name_data['ids']):
                        if sid == sentence_id:
                            cluster_captions.append(name_data['documents'][i].document)
                            break
                all_name_clusters_captions.append(cluster_captions)
            
            # 各nameクラスタのフォルダ名を生成し、リーフフォルダとして作成
            for cluster_idx, (name_idx, sentence_ids_in_name) in enumerate(name_clusters.items()):
                ...
                name_captions = all_name_clusters_captions[cluster_idx]
                ...
                if len(sibling_captions) > 0:
                    ...
                    name_folder_name = self._get_folder_name_with_sibling_comparison(
                        name_captions,
                        sibling_captions,
                        ['object','main','its','the'] + MAJOR_COLORS + MAJOR_SHAPES
                    )
                    print(f"    📁 リーフフォルダ名生成（同階層比較）: '{name_folder_name}' (ID: {name_folder_id})")
                else:
                    # 1つしかない場合は通常のTF-IDF
                    name_folder_name = self._get_folder_name(name_captions, ['object','main','its','the'] + MAJOR_COLORS + MAJOR_SHAPES)
                    print(f"    📁 リーフフォルダ名生成（TF-IDF）: '{name_folder_name}' (ID: {name_folder_id})")
                ...
                print(f"    ✅ リーフフォルダ作成完了: '{name_folder_name}' (画像数: {len(leaf_data)})")
            ...
            # usage+categoryフォルダを作成してnameフォルダをその下に配置
